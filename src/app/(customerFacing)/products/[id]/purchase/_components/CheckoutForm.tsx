    ...
    <div className="max-w-5xl w-full mx-auto space-y-8">
        <div className="flex gap-4 items-center">
            <div className="aspect-video flex-shrink-0 w-1/3 relative">
                <Image src={product.imagePath} fill alt={product.name} className="object-fit" />
            </div>
            <div>
        ...
        <Elements options={{clientSecret}} stripe={stripePromise}>
            <Form priceInCents={product.priceInCents} productId={product.xata_id} />
        </Elements>
    </div>
...
function Form({ priceInCents, productId } : { priceInCents: number, productId: string }) {
   ...
   async function handleSubmit(e: FormEvent) {
    ...
    if(orderExists) {
        setErrorMessage("You have alredy purchased this product.")
        setIsLoading(false)
        return
    }
        ...
        <Card>
            ...
            <CardFooter>
                ...
                disabled={stripe == null || elements == null || isloading} >
                    {isloading
                    ? "Purchasing..."
                    : `Purchase - ${formatCurrency(priceInCents / 100)}`}</Button>
            </CardFooter>
        </Card>
        </form>
    )     
